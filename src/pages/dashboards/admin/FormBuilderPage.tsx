...
export const FormBuilderPage: React.FC = () => {
  const { projectTypeId, templateId } = useParams<{
  ...
  const {
    ...
    loading,
    error,
  } = useReportTemplateDetails({ templateId: templateId || "" });
  ...
  useEffect(() => {
    if (reportTemplate) {
      if (reportTemplate.sections && reportTemplate.sections.length > 0) {
        ...
        const convertedCategories: FormCategory[] = reportTemplate.sections.map(
  ...
  if (error || !reportTemplate) {
    return (
      <div className="text-center py-12">
        ...
        <h3 className="text-heading-3 text-secondary-900 mb-2">
